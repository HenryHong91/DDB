...
export default function Product() {
  const getData = () => {
    axios.get("https://localhost:7247/api/Product").then((res) => {
      setData(res.data);
    });
  };
  useEffect(() => {
  ...
  const handleEdit = (id) => {
    setOpen(true);
    axios.get(`https://localhost:7247/api/Product/${id}`).then((res) => {
  ...
  const handleUpdate = () => {
    const url = `https://localhost:7247/api/Product/${selectedId}`;
  ...
  return (
    <TableContainer component={Paper}>
      <Box display="flex" justifyContent="space-between" alignItems="center">
        <h1 style={{ marginLeft: "20px" }}> Product</h1>
        <Button
          variant="contained"
          size="large"
          ...
          <Box sx={style}>
            ...
            >
              <Button
                ...
                onClick={() => handleUpdate(selectedId, name, qty, price)}
              >
                Edit
              </Button>
              <Button
