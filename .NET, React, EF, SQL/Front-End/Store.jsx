...
export default function Store() {
  ...
  const getData = () => {
    axios
      .get("https://localhost:7247/api/Store")
      .then((res) => {
        setData(res.data);
      })
      .catch((error) => {
  ...
  const handleEdit = (id) => {
    setOpen(true);
    setOpen(true);
    axios.get(`https://localhost:7247/api/Store/${id}`).then((res) => {
  ...
  const handleUpdate = () => {
    const url = `https://localhost:7247/api/Store/${selectedId}`;
  ...
  const handleCreate = () => {
    const url = "https://localhost:7247/api/Store";
    const newClient = {
      id: uuidv4(),
      name,
      street,
  ...
  const handleDelete = (name, id) => {
    if (window.confirm(`${name} will be deleted`)) {
      ...
      axios
        .delete(`https://localhost:7247/api/Store/${id}`)
        .then((response) => {
            ...
            <Box sx={style}>
              ...
              >
                <Button
                  ...
                  onClick={() =>
                    handleUpdate(selectedId, name, street, city, zipcode)
                  }
                >
                  Edit
                </Button>
